...
- Writes a line-oriented log compatible with typical Tracenav log style:
  "# Format: [unix_ms] [type] [subtype] [length] [hex_data]"
  where unix_ms is the UTC Unix epoch timestamp in milliseconds.
...
- The script rotates to a new file every --log-duration seconds (default 3600 = 1 hour).
...
try:
    ...
    pass
from datahandler.spartnPress.decode_spartn_from_pmp import parse_spartn_frame  # type: ignore

# Hardcoded GGA parameters - easily configurable
LATITUDE = 38.3032          # Latitude in decimal degrees (Fredericksburg, VA)
LONGITUDE = -77.4605        # Longitude in decimal degrees (Fredericksburg, VA)
ALTITUDE = 30.0             # Altitude above sea level in meters
...
LOG_FILE_DURATION = 3600  # seconds per file (1 hour)
...
def main():
    ...
    while True:  # Main reconnection loop
        ...
        try:
            ...
            log_file_txt.write(f"# GGA Position: {LATITUDE}, {LONGITUDE}\n")
            log_file_txt.write("# GPS Week: 0\n")
            log_file_txt.write("# GPS TOW: 0ms\n")
            log_file_txt.write("# Format: [unix_ms] [type] [subtype] [length] [hex_data]\n")
            log_file_txt.write("# ----------------------------------------\n")
            log_file_txt.flush()
            ...
            while True:  # Data reception loop
                ...
                while True:
                    ...
                    line = f"{unix_ms} {type_field} {subtype} {len(frame)} {frame.hex()}\n"
                    if log_file_txt:
