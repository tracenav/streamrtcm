  ...
  while True:
    try:
      ...
      while True:
        ...
        # periodic NMEA (GGA + ZDA) to ESP32 for PPL
        now_ts = time.time()
        # NMEA at ~1Hz scaled by speed (2x => 2 per sec)
        nmea_period = 1.0 / speed
        if (now_ts - last_nmea_serial) >= nmea_period:
          try:
            gga_line = gga_fn()
            zda_line = build_zda()
            ser.write(gga_line.encode('ascii'))
            ser.write(zda_line.encode('ascii'))
            # Compose display time based on PMP base timestamp if available
            disp_mmdd = None
            disp_hms = None
            if use_pmp_dir and (pmp_absolute_base_ms is not None or pmp_base_tod_ms is not None):
              elapsed_ms = 0
              if pmp_start_time is not None:
                elapsed_ms = int((time.time() - pmp_start_time) * 1000.0)
              if pmp_absolute_base_ms is not None:
                t_abs_ms = pmp_absolute_base_ms + elapsed_ms
                dt = datetime.datetime.fromtimestamp(t_abs_ms / 1000.0, tz=datetime.UTC)
                disp_mmdd = dt.strftime('%m%d')
                disp_hms = dt.strftime('%H:%M:%S')
              else:
                base_ms = pmp_base_tod_ms or 0
                combined = base_ms + elapsed_ms
        ...
        if use_spartn_file or use_pmp_file or use_pmp_dir:
          ...
          if use_spartn_file and spartn_file_frames:
            if spartn_base_ts is None:
              ...
              spartn_start_time = time.time()
              spartn_index = 0
            # send due spartn frames
            ...
            if first_ts is not None:
              ...
              now0 = pmp_start_time if pmp_start_time is not None else time.time()
              pmp_start_time = now0
